...
void setup_timer1(void){ 
  ...
  TCCR1A = (0b11 << WGM10) | (0b00 << COM1A0) | (0b00 << COM1B0) | (0b00 << COM1C0); // low WGM bits & outputs 
...
void setup() {
  ...
  setup_timer1();
  freqHzD5 = 50000; // Fixed-point value for Hertz with 5 decimal places at 0.00001Hz resolution
  accum=0;
}
...
void loop() {
  ...
  unsigned long duration, now_millis ;
  char buff[180];
 ...
 // while(digitalRead(toggle_pin) == last_state) ;  // delay
  ...
  if (digitalRead(toggle_pin) != last_state){
    ...
    duration = millis()-last_change;
    last_change = millis(); 
  }
  //*portInputRegister(digitalPinToPort(toggle_pin)) = digitalPinToBitMask(toggle_pin);
  //delay(100);
  Serial.print(1.0*millis());
  Serial.print("us, ");
  sprintf(buff,"%lu HzD5, %lu steps",freqHzD5,TOGGLE/freqHzD5);
  Serial.print(buff);
  Serial.print(" in ");
  ...
  Serial.print(' ');
  Serial.print(" (TIMER0: 0x");
  Serial.print(TCCR0A,HEX);
  Serial.print(" 0x");
  Serial.print(TCCR0B,HEX);
  Serial.print(" 0x");
  Serial.print(TIMSK0,HEX);
  Serial.print(")");
  Serial.print(" (TIMER1: 0b");
